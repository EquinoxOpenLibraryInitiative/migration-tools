...
foreach my $fp ( keys %incoming ) {
	...
	if (defined $pines{ $fp }) { # match!
		...
		foreach my $id ( @{ $incoming{ $fp } } ) {
			...
			print RECORD_IDS "$id\n";
			if ( ! defined $candidate_match{ $id } ) { $candidate_match{ $id } = []; }
...
foreach my $id ( keys %candidate_match ) {
	...
	my $subtitle; if (defined $score{ $id }) { $subtitle = $score{ $id }[1]; }
	...
	foreach my $fp ( @fps ) {
		foreach my $pines_id ( @{ $pines{ $fp } } )  {
			my $pines_subtitle; if (defined $score{ $pines_id }) { $pines_subtitle = $score{ $pines_id }[1]; }
			my $pines_holdings; if (defined $score{ $pines_id }) { $pines_holdings = $score{ $pines_id }[0]; }
	...
	if (! defined $match{ $best_pines_id } ) { $match{ $best_pines_id } = [ $best_pines_id ]; }
