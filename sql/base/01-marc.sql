...
-- removes tags based on text in any subfield 
-- examples strip_tag(marc,'856','rbdigital')
DROP FUNCTION IF EXISTS migration_tools.strip_tag(TEXT,TEXT,TEXT);
CREATE OR REPLACE FUNCTION migration_tools.strip_tag(marc TEXT, tag TEXT, evidence TEXT)
 RETURNS TEXT
 LANGUAGE plperlu
AS $function$
use strict;
...
foreach my $f (@fields) {
    my $str = lc($f->as_string());
    if ($str =~ m/$evidence/) { push @fields_to_delete, $f; }
}
