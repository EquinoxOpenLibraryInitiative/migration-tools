...
-- SELECT * FROM migration_tools.fields_requiring_mapping;
-- \d foo.actor_usr
...
CREATE TABLE migration_tools.config (
    key TEXT UNIQUE,
    value TEXT
);
...
CREATE OR REPLACE FUNCTION migration_tools.build_default_base_staging_tables (TEXT) RETURNS VOID AS $$
    ...
    BEGIN
        ...
        SELECT migration_tools.production_tables() INTO STRICT production_tables;
...
CREATE OR REPLACE FUNCTION migration_tools.build_specific_base_staging_table (TEXT,TEXT) RETURNS VOID AS $$
    ...
    BEGIN
        ...
        INSERT INTO migration_tools.fields_requiring_mapping
